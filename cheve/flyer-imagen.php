...
$tipo  = isset($_GET['tipo']) && $_GET['tipo'] === 'logo' ? 'logo' : 'foto';
...
$archivo = ($tipo === 'logo' && !empty($flyer['logo_organizador'])) ? $flyer['logo_organizador'] : $flyer['imagen'];
$ruta    = flyer_ruta_imagen($archivo);

if ($ruta === '') {
    http_response_code(404);
    exit('Este flyer no tiene imagen guardada.');
}
