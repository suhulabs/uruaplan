...
function c($seccion, $campo, $porDefecto = '')
{
    ...
    static $jsonDefault = null;
    if ($jsonDefault === null) {
        $jsonDefault = leer_json(DATA_DIR . '/contenido.json', array());
    }
    if (isset($jsonDefault[$seccion][$campo]) && is_scalar($jsonDefault[$seccion][$campo])) {
        $valor = (string) $jsonDefault[$seccion][$campo];
        if ($valor !== '') {
            return $valor;
        }
    }
...
function lista($seccion, $clave)
{
    ...
    if (isset($contenido[$seccion][$clave]) && is_array($contenido[$seccion][$clave])) {
        $filas = array_values(array_filter($contenido[$seccion][$clave], 'is_array'));
    }
    ...
    if (empty($filas)) {
        ...
        if (isset($jsonDefault[$seccion][$clave]) && is_array($jsonDefault[$seccion][$clave])) {
            $filas = array_values(array_filter($jsonDefault[$seccion][$clave], 'is_array'));
        }
    }
...
function cimg($seccion, $campo, $porDefecto = '', $prefijo = '')
{
    ...
    // Si la ruta obtenida de la base de datos no existe físicamente en disco,
    // recurre automáticamente al archivo de contenido predeterminado contenido.json.
    if ($val === '' || !is_file(SITIO_DIR . '/' . ltrim($val, '/'))) {
        static $jsonDefault = null;
        if ($jsonDefault === null) {
            $jsonDefault = leer_json(DATA_DIR . '/contenido.json', array());
        }
        $valDef = isset($jsonDefault[$seccion][$campo]) ? (string) $jsonDefault[$seccion][$campo] : '';
        if ($valDef !== '' && is_file(SITIO_DIR . '/' . ltrim($valDef, '/'))) {
            $val = $valDef;
        }
    }
    ...
    $url = url_activo($val, $prefijo);
    if ($url !== '') {
        $rutaDisco = SITIO_DIR . '/' . ltrim($val, '/');
        $version = is_file($rutaDisco) ? filemtime($rutaDisco) : time();
        $url .= '?v=' . $version;
    }
