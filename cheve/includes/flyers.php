...
/**
 * Migración silenciosa de columnas para garantizar que la tabla flyers
 * tenga las columnas nuevas aunque venga de una versión previa.
 */
function flyers_asegurar_columnas()
{
    ...
    try {
        ...
        $cols = bd_filas("SHOW COLUMNS FROM flyers LIKE 'logo_organizador'");
        if (empty($cols)) {
            bd_ejecutar("ALTER TABLE flyers ADD COLUMN logo_organizador VARCHAR(190) NOT NULL DEFAULT ''");
        }
        $colsPub = bd_filas("SHOW COLUMNS FROM flyers LIKE 'logo_organizador_publico'");
        if (empty($colsPub)) {
            bd_ejecutar("ALTER TABLE flyers ADD COLUMN logo_organizador_publico VARCHAR(190) NOT NULL DEFAULT ''");
        }
        $colsGeo = bd_filas("SHOW COLUMNS FROM flyers LIKE 'geo_origen'");
        if (empty($colsGeo)) {
            bd_ejecutar("ALTER TABLE flyers ADD COLUMN geo_origen VARCHAR(150) NOT NULL DEFAULT ''");
        }
    } catch (Throwable $e) {
        error_log('[uruaplan] Auto-migración columnas flyers: ' . $e->getMessage());
    }
}
    ...
    bd_ejecutar(
        'INSERT INTO flyers
            (evento, subtitulo, fecha, hora, ubicacion, precio, contacto,
             ...
             imagen_alto, imagen_bytes, logo_organizador, logo_organizador_publico,
             nota, ip, agente, geo_origen, correo_ok,
        ...
        array(
            ...
            (string) flyer_valor($datos, 'logo_organizador'),
            '',
            '',
            substr($ip, 0, 45),
            substr($agente, 0, 255),
...
function flyer_publicar($id, array $usuario)
{
    ...
    if (!empty($flyer['logo_organizador'])) {
        $origenLogo = flyer_ruta_imagen($flyer['logo_organizador']);
        if ($origenLogo !== '' && is_file($origenLogo)) {
            $extLogo = pathinfo($flyer['logo_organizador'], PATHINFO_EXTENSION) ?: 'png';
            $nombreLogo = 'logo-org-' . (int) $flyer['id'] . '-' . bin2hex(random_bytes(4)) . '.' . $extLogo;
            if (@copy($origenLogo, UPLOADS_DIR . '/' . $nombreLogo)) {
                @chmod(UPLOADS_DIR . '/' . $nombreLogo, 0644);
                $logoPublicoRuta = UPLOADS_REL . '/' . $nombreLogo;
            }
        }
    }

    try {
        bd_transaccion(function () use ($flyer, $rutaPublica, $logoPublicoRuta, $usuario) {
            bd_ejecutar(
                'UPDATE flyers SET publicado = 1, imagen_publica = ?, logo_organizador_publico = ?, actualizado = ? WHERE id = ?',
                array($rutaPublica, $logoPublicoRuta, date('Y-m-d H:i:s'), (int) $flyer['id'])
            );

            flyer_anotar($flyer['id'], 'publicado', $usuario, 'Ya se ve en la cartelera de la web.');
        });
    } catch (BdError $e) {
        // Si la base falla, las copias públicas no deben quedarse sueltas.
        borrar_subida_si_procede($rutaPublica);
        if ($logoPublicoRuta !== '') {
            borrar_subida_si_procede($logoPublicoRuta);
        }
        return $e->getMessage();
    }
...
function flyer_despublicar($id, array $usuario)
{
    ...
    bd_transaccion(function () use ($flyer, $usuario) {
        bd_ejecutar(
            "UPDATE flyers SET publicado = 0, imagen_publica = '', logo_organizador_publico = '', actualizado = ? WHERE id = ?",
...
function flyer_quitar_copia_publica(array $flyer)
{
    ...
    if (!empty($flyer['logo_organizador_publico'])) {
        borrar_subida_si_procede($flyer['logo_organizador_publico']);
    }
}
