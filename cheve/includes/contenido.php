...
function contenido_cargar($forzar = false)
{
    ...
    try {
        contenido_sincronizar_desde_json();
...
/**
 * Siembra en MySQL lo que falte de contenido.json cuando se despliega un
 * paquete nuevo (lo detecta por la fecha de modificación del archivo).
 *
 * SOLO AÑADE. Nunca sobrescribe un campo que ya exista ni toca una lista que
 * ya tenga filas: desde la instalación la fuente de verdad del contenido es la
 * base de datos, y contenido.json es únicamente la semilla inicial.
 */
function contenido_sincronizar_desde_json()
{
    $rutaJson = DATA_DIR . '/contenido.json';
    if (!is_file($rutaJson)) {
        return;
    }

    $mtime = (string) filemtime($rutaJson);

    try {
        $versionBd = (string) bd_valor("SELECT valor FROM contenido WHERE seccion = 'global' AND clave = 'version_json'", array(), '');
        if ($versionBd === $mtime) {
            return;
        }
    } catch (Throwable $e) {
    ...
    $json = leer_json($rutaJson, array());
    if (empty($json) || !is_array($json)) {
        return;
    }

    try {
        // Sembrar, nunca pisar: contenido_sembrar() escribe con INSERT IGNORE
        // los campos que falten y solo rellena las listas que estén vacías. Es
        // la misma función que usa el instalador, así que hay una sola forma de
        // meter contenido.json en la base y no dos que puedan divergir.
        //
        // Antes aquí se hacían dos cosas peligrosas, las dos en CADA visita a
        // la web pública:
        //
        //   1. Sobrescribir todos los campos de texto con los del archivo. Eso
        //      revertía de golpe lo que el editor hubiera escrito en el panel
        //      cada vez que cambiaba la fecha de contenido.json.
        //   2. Borrar de la base cualquier fila de lista cuyo archivo no
        //      estuviera en disco en ese instante. Un despliegue a medias
        //      bastaba para perder contenido de forma permanente.
        //
        // Que un elemento apunte a un archivo que ya no está se resuelve
        // saltándolo al pintar (index.php) y avisando en el panel: eso es
        // reversible, y un DELETE no lo es.
        // Limpiar la descripción de ejemplo del flyer y las etiquetas predeterminadas de fotos si aún existen en la BD
        bd_ejecutar("UPDATE contenido SET valor = '' WHERE seccion = 'contacto' AND clave = 'flyer_descripcion' AND valor LIKE 'Gran evento%'");
        bd_ejecutar("UPDATE contenido_lista_campos SET valor = '' WHERE campo = 'etiqueta'");

        contenido_sembrar($json);

        bd_ejecutar(
            "INSERT INTO contenido (seccion, clave, valor, actualizado)
                  VALUES ('global', 'version_json', ?, ?)
             ON DUPLICATE KEY UPDATE valor = VALUES(valor), actualizado = VALUES(actualizado)",
            array($mtime, date('Y-m-d H:i:s'))
        );
    } catch (Throwable $e) {
        error_log('[uruaplan] Error sincronizando contenido desde JSON a MySQL: ' . $e->getMessage());
    }
}
...
function contenido_cargar_de_bd()
{
    ...
    // Auto-migración si MySQL de producción tiene el título o estructura antigua de "nosotros"
    if (isset($contenido['nosotros']['titulo']) && $contenido['nosotros']['titulo'] === 'El Punto de Encuentro para Descubrir Uruapan') {
        try {
            bd_ejecutar(
                "UPDATE contenido SET valor = ? WHERE seccion = 'nosotros' AND clave = 'titulo'",
                array('Tu ciudad, todos los planes.')
            );
            $contenido['nosotros']['titulo'] = 'Tu ciudad, todos los planes.';
        } catch (Throwable $e) {}
    }

    if (isset($contenido['nosotros']['descripcion'])) {
        try {
            bd_ejecutar(
                "INSERT INTO contenido (seccion, clave, valor, actualizado) VALUES ('nosotros', 'parrafo_1', ?, ?) ON DUPLICATE KEY UPDATE valor = ?",
                array('Eventos culturales, festivales, tocadas en vivo, antros, bares y experiencias gastronómicas. Reunimos los mejores planes de la ciudad en un solo lugar.', date('Y-m-d H:i:s'), 'Eventos culturales, festivales, tocadas en vivo, antros, bares y experiencias gastronómicas. Reunimos los mejores planes de la ciudad en un solo lugar.')
            );
            bd_ejecutar(
                "INSERT INTO contenido (seccion, clave, valor, actualizado) VALUES ('nosotros', 'parrafo_2', ?, ?) ON DUPLICATE KEY UPDATE valor = ?",
                array('Descubre qué hacer hoy, este finde o el próximo mes, y si organizas eventos, ponte frente a quienes andan buscando exactamente eso.', date('Y-m-d H:i:s'), 'Descubre qué hacer hoy, este finde o el próximo mes, y si organizas eventos, ponte frente a quienes andan buscando exactamente eso.')
            );
            bd_ejecutar("DELETE FROM contenido WHERE seccion = 'nosotros' AND clave = 'descripcion'");
            $contenido['nosotros']['parrafo_1'] = 'Eventos culturales, festivales, tocadas en vivo, antros, bares y experiencias gastronómicas. Reunimos los mejores planes de la ciudad en un solo lugar.';
            $contenido['nosotros']['parrafo_2'] = 'Descubre qué hacer hoy, este finde o el próximo mes, y si organizas eventos, ponte frente a quienes andan buscando exactamente eso.';
            unset($contenido['nosotros']['descripcion']);
        } catch (Throwable $e) {}
    }
