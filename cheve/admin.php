...
// --- Elementos de las listas que apuntan a un archivo que ya no está -------
// La web se los salta al pintar para no enseñar imágenes rotas, y eso, sin un
// aviso, es una desaparición silenciosa: la casilla deja de salir y nadie sabe
// por qué. Aquí se dicen en voz alta, con su ruta, para poder arreglarlos.
// Nada se borra: la fila sigue en la base con su etiqueta y su encuadre, así
// que basta con volver a subir el archivo para recuperarla.
...
foreach ($secciones as $idSeccion => $defSeccion) {
    ...
    foreach ($defSeccion['listas'] as $claveLista => $defLista) {
        if (!isset($contenido[$idSeccion][$claveLista]) || !is_array($contenido[$idSeccion][$claveLista])) {
            continue;
        }

        foreach ($contenido[$idSeccion][$claveLista] as $fila) {
            if (!is_array($fila)) {
                continue;
            }

            foreach ($defLista['campos'] as $clave => $defCampo) {
                if (!es_campo_archivo(isset($defCampo['tipo']) ? $defCampo['tipo'] : 'texto')) {
                    continue;
                }
                if (empty($fila[$clave])) {
                    continue;
                }
                if (!is_file(SITIO_DIR . '/' . ltrim((string) $fila[$clave], '/'))) {
                    $archivosFaltantes[] = $defLista['titulo'] . ' → ' . $fila[$clave];
                }
            }
        }
    }
...
if ($archivosFaltantes) {
    $problemas[] = '<strong>' . count($archivosFaltantes) . ' elemento(s) no se están viendo en la web</strong> '
        . 'porque su archivo no existe en el servidor: <code>'
        . implode('</code>, <code>', array_map('e', $archivosFaltantes)) . '</code>. '
        . 'Vuelve a subir el archivo en la fila correspondiente, o quita la fila si ya no lo quieres.';
}
